...
def sieve(max)
  ...
  num = 2
  while num <= max do
    primes << num if !(2 .. Math.sqrt(num).to_i).any? {|prime| num % prime == 0}
    num +=1
  end
...
(0 .. inf).each do |n|
  if factors(triangular(n)).length >= 500 
    #puts "#{triangular(n)} , #{factors(triangular(n)).length}, #{factors(triangular(n))}"
    puts triangular(n)
    break
  end
end
...
(1..1000000).each {|n| 
  temp = [n]
  temp << collatz(temp[-1]) while collatz(temp[-1])!=1
